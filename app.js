const BASE_URL = "http://localhost:3001/student"; // 기본 주소 설정
...
document.getElementById('student-form').addEventListener('submit', async (e) => {
    ...
    try {
        await fetch(BASE_URL, { // 기본 주소로 POST 요청
            method: 'POST',
            headers: {
...
document.getElementById('student-update-form').addEventListener('submit', async (e) => {
    ...
    try {
        await fetch(`${BASE_URL}/${id}`, { // ID를 URL에 포함하여 요청합니다.
            method: 'PUT',
            headers: {
                "Content-Type": "application/json"
            },
...
// Delete student
document.getElementById('student-delete-form').addEventListener('submit', async (e) => {
    e.preventDefault();
